...
class ranking():
    ...
    #                                                       'output-csv':'output.csv'}):
        ...
        self.doe_obj.set_selected_conditions(final_exp_dataframe)
        self.doe_obj.set_selected_yamls(list(final_exp_dataframe['experiment']))
        self.doe_obj.set_selected_matrices(self.updated_S,new_Z,new_Y)
    ...
    def get_rankings(self,excluded_yamls,S,iteration,countH=0,countV=0,Z_prev=None,Y_prev=None,X_prev=None):
        ...
        elif re.match('[Ii]nformation[-_ ][Ee]ntropy',self.inputs['quantity_of_interest']):
            original_covar_det=np.linalg.slogdet(self.doe_obj.covar_original)[1]
        ...
        for i,file in enumerate(self.doe_obj.experiment_yamls):
            ...
            if file not in excluded_yamls:
                ...
                if re.match('[Rr]ate[-_ ][Cc]onstant',self.inputs['quantity_of_interest']):
                    ...
                    dets_list=dets_list+[np.linalg.slogdet(self.updated_c)[1]]
                    np.set_printoptions(threshold=np.inf)
                    #print(np.linalg.slogdet(self.updated_c)[1])
                    #print(dets_list)
                elif re.match('[Ii]gnition[-_ ][Dd]elay',self.inputs['quantity_of_interest']):
                    ...
                    dets_list=dets_list+[np.linalg.slogdet(self.updated_c)[1]]
                    ranking_list=ranking_list+[None]
        ...
        #sub.finish()
        #print(ranking_list)
        output_ranking=pd.DataFrame(columns=['experiment','ratio','determinant'])
        ...
        output_ranking['determinant']=dets_list
        ...
        output_ranking.sort_values(by='determinant',ascending=True,inplace=True)
